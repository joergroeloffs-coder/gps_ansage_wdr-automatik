...
- `departureDetection`: globale Ablege-Erkennung (nicht pro Station, siehe unten):
  ...
  - `stableDurationMinutes` (Default 1,5 = 90 Sekunden): So lange muss das Schiff innerhalb von `stableRadiusMeters` bleiben, damit die Position als Ablege-Anker gilt ("angelegt").
...
3. Die Hintergrundfarbe der App (`fleet[].bgColor` in `stations.json`) – rein optisch, damit auf einen Blick erkennbar ist, für welches Schiff gerade eingestellt ist (Schleswig-Holstein blau, Nordfriesland gelb, Norderaue hellgrau, Uthlande rot).
4. Welcher Fahrplan-Eintrag für die Automatik herangezogen wird.
...
**Sortierung der Hafen-Karten:** Die drei Karten unter "Einlaufen" ordnen sich automatisch danach, welcher Hafen laut Fahrplan als nächstes für das gewählte Schiff relevant ist (kürzeste Zeit bis zur nächsten Abfahrt ab diesem Hafen) – der relevanteste Hafen steht oben. Aktualisiert sich beim Schiffswechsel, beim Laden des Fahrplans und einmal pro Minute.
